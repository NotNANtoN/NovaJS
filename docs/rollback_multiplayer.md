...
**Phase 2 is complete.** Flagged for Phase 3: on the `node` platform, `ShipController` skips building the control systems, so *server* sim worlds run a different system set than client workers — a cross-peer determinism hazard the input-exchange protocol must resolve (all peers must run identical systems). Node test harnesses now use the `'worker'` platform (with a disk-backed `getSettings`) to match the real client sim. Also parked: on player death, restore the player's entity state from a snapshot taken at launch/jump-in (per-entity restore is a natural extension of the snapshot machinery) instead of teleporting to the origin.
